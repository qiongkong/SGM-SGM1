...
// 左到右，位置偏移就是列号加1；上到下，位置偏移就是行号加1；对角线路径中的左上到右下，位置偏移就是行号列号各加1。
// 每个路径前后像素的代价和灰度的位置偏移量都是固定的，比如左到右偏移量是1个像素(disp_range个代价)；上到下偏移量是 w 个像素(w * disp_range 个代价)；左上到右下偏移量是 (w + 1) 个像素((w+1) * disp_range个代价)。
// 边界时，行号继续按方向走一步，列号重置为起始列号
// 对角线1，左上/右下
void utils::CostAggregateDiagonal_1(const uint8* img_data, const sint32& width, const sint32& height,
	...
	for (sint32 j = 0; j < width; j++) {
		...
		// 对角线路径上的下一个像素，相隔width+1个像素
		// 增加边界处理，行号按原方向继续前进，列号跳到另一边界
		current_row = is_forward ? 0 : height - 1;  // 行
		current_col = j;  // 列
